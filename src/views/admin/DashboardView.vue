                ...
                <div class="flex items-start gap-5 flex-1 w-full xl:w-auto">
                   ...
                   <div class="min-w-0 flex-1">
                      ...
                      <div class="flex flex-wrap items-center gap-2 mt-3 text-[11px] font-bold uppercase tracking-wider">
                         ...
                         <span class="bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded border border-indigo-100" title="Batas Maksimum Unit">
                           Max: {{ item.maxStock }}
                         </span>
                         <span 
                       ...
                       <div v-if="item.action === 'approve'" class="bg-white p-4 rounded-xl border border-emerald-200 shadow-sm flex items-center justify-between gap-4 animate-fade-in-up">
                           ...
                           <div class="flex-1">
                              ...
                              <p v-else-if="item.approved_qty < item.reqQty" class="text-[10px] text-orange-500 mt-1 font-bold flex items-center gap-1">
                                 <ExclamationCircleIcon class="h-3 w-3" /> Parsial (Kurang dari Req)
                              </p>
                           </div>
                       </div>

                       <div v-else class="bg-white p-4 rounded-xl border border-red-200 shadow-sm animate-fade-in-up h-full">
                           ...
                           <textarea 
                              v-model="item.reason" 
                              rows="2" 
                              class="w-full rounded-lg border-red-200 bg-red-50/10 text-xs text-slate-700 shadow-inner focus:border-red-500 focus:ring-red-500 placeholder:text-slate-400 p-2 resize-none"
                    ...
                    <div class="flex items-start gap-4 flex-1">
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <div class="p-3 bg-slate-50 border border-slate-200 rounded-xl relative group-hover:bg-white transition-colors">
                          ...
                          <p v-else class="text-[10px] text-slate-400 pl-3 mt-1 italic">Tidak ada alasan/keterangan.</p>
                        </div>
                      </div>
                    </div>
...
const openApprovalModal = (trx) => {
    // Siapkan data per item, hitung sisa stok dll
...
const validateItem = (item) => {
    // Helper untuk auto-correct input jika berlebih
    if (item.approved_qty > item.maxAllocatable) {
        // Optional: bisa auto set, atau biarkan error muncul
        // item.approved_qty = item.maxAllocatable;
    }
}

// Validasi Global Form Modal
const isFormValid = computed(() => {
    if (!approvalModal.value.items.length) return false;
    
    for (const item of approvalModal.value.items) {
        if (item.action === 'approve') {
            // Jika terima: Qty tidak boleh melebihi slot
...
const submitBatchProcessing = () => {
    ...
    // Pastikan store memiliki method 'processBatchTransaction' yang menerima (trxId, itemsArray)
